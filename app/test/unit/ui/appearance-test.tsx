...
function renderAppearance(
  ...
  // This fork adds its own sections to the Appearance tab (Madness themes,
  // personality, notifications, accessibility, changes list), so the fixture
  // carries more than upstream's does.
  const props = {
    selectedTheme: ApplicationTheme.Light,
    onSelectedThemeChanged: () => {},
    selectedMadnessTheme: '' as MadnessTheme,
    onSelectedMadnessThemeChanged: () => {},
    selectedPersonality: '' as MadnessPersonality,
    onSelectedPersonalityChanged: () => {},
    selectedTabSize: 4,
    ...
    onShowDiffCheckMarksChanged: () => {},
    groupChangesByFolder: true,
    onGroupChangesByFolderChanged: () => {},
  }
...
describe('Appearance preferences', () => {
  ...
  it('hides the worktree preference when worktrees are disabled', () => {
    renderAppearance(false, false)

    assert.strictEqual(
      screen.queryByRole('checkbox', { name: 'Always show worktree list' }),
      null
    )
    assert.ok(screen.getByRole('heading', { name: 'Miscellaneous' }))
  })
})
