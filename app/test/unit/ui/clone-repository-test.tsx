...
import { getDesktopStrings } from '../../../src/ui/lib/theme-strings-context'
...
async function renderCloneDialog(
  ...
  // Upstream hardcodes "Clone" here. This fork takes the label from the
  // personality strings, where it is "Clone repository" by default and
  // something else entirely under other personalities, so ask the same source
  // the dialog asked rather than pinning a literal.
  const cloneButton = screen.getByRole('button', {
    name: getDesktopStrings().cloneRepository,
    hidden: true,
  })
...
describe('CloneRepository path validation', () => {
  ...
  it('validates a path chosen through the macOS directory picker', async t => {
    ...
    // Upstream mocks electron's ipcRenderer. This fork's renderer reaches the
    // main process through window.electronBridge (lib/ipc-renderer), so the
    // dialog's invoke has to be intercepted there or the real one runs.
    t.mock.method(window.electronBridge, 'invoke', async (channel: string) => {
