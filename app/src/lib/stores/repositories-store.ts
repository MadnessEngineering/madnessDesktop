...
> {
  ...
  public async updateRepositoryPath(
    ...
    // through reselect → refresh → heal indefinitely. Every field the
    // update below writes has to be compared here, or a change to just that
    // field would be silently dropped.
    if (
      repository.path === path &&
      repository.gitDir === gitDir &&
      repository.mainWorktreePath === mainWorktreePath &&
