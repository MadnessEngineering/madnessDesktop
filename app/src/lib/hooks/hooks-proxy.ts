  ...
  return async (conn: Connection) => {
    ...
    if (hasStdin && stdinPath) {
      try {
        ...
        // This fork's HookProgress carries the fields the hook log renders, so
        // a failure before the hook ever spawned still has to report them. The
        // hook produced no output and no exit code of its own; 1 stands in, and
        // the reason goes in `output` so the log says why.
        onHookProgress?.({
          hookName,
          status: 'failed',
          exitCode: 1,
